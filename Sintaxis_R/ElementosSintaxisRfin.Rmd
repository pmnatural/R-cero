...
Una función nueva se crea mediante la siguiente construcción:

function (arglist) {body}

Dentro de los {} va el codigo que define el cuerpo de la funcion. 

En R usamos muchas funciones que ya vienen listas en los paquetes, y lo unico que tenemos que preocuparnos es de pasarle los argumentos correctos (arglist) y en usar o almacenar el valor que devuelva.

*summary*: funcion para contar cantidad de ocurrencias en cada categoria
Por ejemplo aplicado a vector de caracteres: 
```{r}
v <- c("j", "k", "l")
summary(v)
```

Si lo usamos con el dataset *iris*
```{r}
summary(iris)
```
