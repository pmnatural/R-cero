...
![](C:/Users/M-C/Documents/R/Curso Monica Alonso/ojos.png) R es case sensitive, quiere decir importa mucho si usas mayúscula o minúscula. Si en la asignación escribiste VariableX (con mayúscula) pero luego tipeás Variablex (con minúscula) te va a indicar que no conoce esa variable
...
Se puede saber cuál es la ubicación que estamos usando como directorio de trabajo  mirando la barra de titulos de la consola. Pero si queremos usar esta ubicación en una parte de un código, usamos este comando (también desde la desde la consola, probalo). 
```{r}
getwd()
```
...
Si queres remover un objeto que creaste (ej *x*), haces 
```{r}
rm(objeto) #reemplazá con el nombre de algun objeto/variable creada por vos
```
